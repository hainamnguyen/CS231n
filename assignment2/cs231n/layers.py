...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #Normalise our batch then shift with gamma/beta.
...
def batchnorm_backward_alt(dout, cache):
    ...
    #Get catched variables from forward pass.
    N, D = dout.shape
    normalized_data = cache.get('normalized_data')
    gamma = cache.get('gamma')
    ivar = cache.get('ivar')
    ...
    #Backdrop dout to calculate dbeta and gamma.
    dbeta = np.sum(dout, axis = 0)
    dgamma = np.sum(dout * normalized_data, axis = 0)
    
    #Alternate faster formular way of calculating dx.
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        for f in range(F):
            for hh in range(H2):
                for ww in range(W2):
                    out[n, f, hh, ww] = np.sum(x_pad[n, :, hh * stride: hh * stride + H1,
                                  ww * stride : ww * stride + W1] * w[f, :, :, :]) + b[f]
...
def conv_backward_naive(dout, cache):
    ...
    H2 = 1 + int((H + 2 * pad - H1) / stride)
    W2 = 1 + int((W + 2* pad - W1) / stride)
    ...
    for f in range(F):
        for c in range(C):
            for h1 in range(H1):
                for w1 in range(W1):
                    x_pad_tmp = x_pad[:, c, h1 : h1 + H2 * stride : stride, 
                                      w1:w1 + H2 * stride : stride]
