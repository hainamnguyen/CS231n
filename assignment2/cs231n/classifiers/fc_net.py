...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            X_tmp, cache['affine_%d' % (i)] = affine_forward(X_tmp, 
                                                           self.params['W%d' % (i)],
                                                           self.params['b%d' % (i)]
                                                          )
            if self.use_batchnorm:
                X_tmp, cache['batchn_%d' % (i)] = batchnorm_forward(X_tmp,
                                                                    self.params['gamma%d' % (i)], 
                                                                    self.params['beta%d' % (i)],
                                                                    self.bn_params[i - 1])
                
            X_tmp, cache['relu_%d' %(i)] = relu_forward(X_tmp)
            
            #dropout after each ReLu:
            if self.use_dropout:
                X_tmp, cache['dropout_%d' % (i)] = dropout_forward(X_tmp, self.dropout_param)
                
            
        # Forward into last layer
        WLast = 'W%d' % self.num_layers
        bLast = 'b%d' % self.num_layers
        scores, cache_scores = affine_forward(X_tmp,
                                              self.params[WLast],
                                              self.params[bLast])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        for i in reversed(range(1, self.num_layers)):
            ...
            if self.use_dropout:
                dx = dropout_backward(dx, cache['dropout_%d' % i])
                
            dx = relu_backward(dx, cache['relu_%d' % i])
            
            #Batch_norm.
            if self.use_batchnorm:
                dx, grads['gamma%d' % i], grads['beta%d' %i] = batchnorm_backward(dx, cache['batchn_%d' % i])
            dx, grads['W%d' % i], grads['b%d' % i] = affine_backward(dx, cache['affine_%d' % i])
            
            #Add regularizaion term to the gradient
